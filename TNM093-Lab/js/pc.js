 ...
 */
  function pc(data) {
	...
	var foreground = pc_svg.append("g").attr("class", "foreground").selectAll("path").data(data).enter().append("path")
		.attr("d", function (d) { return drawPath(d); });
	//Adds a new tag "g" with the class "foreground", selects all "path" enters the data and appends it to the path with attribute d and value drawpath(d)


	// Task 5.0.9 -- Drawing Axes
	//Selct all elements withc class "dimension", enter the dimension data and append it to the g tag with the class .dimension and .axis
